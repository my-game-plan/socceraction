...
def scores_time_based_window(actions: DataFrame[SPADLSchema], nr_seconds: int=15) -> pd.DataFrame:
    ...
    for i in range(len(actions)):
        ...
        while j < len(actions) and abs(actions['time_seconds'][j] - start_time) < nr_seconds:
            if y['goal'][j] and y['team_id'][j] == possession_team: #team possessing the ball scored a goal
                res.at[i] = True
                break
            elif y['owngoal'][j] and y['team_id'][j] != possession_team: # other team conceded an owngoal
                res.at[i] = True
                break
            j += 1
    return pd.DataFrame(res, columns=['scores'])

def concedes_time_based_window(actions: DataFrame[SPADLSchema], nr_seconds: int=15) -> pd.DataFrame:
    ...
    for i in range(len(actions)):
        ...
        while j < len(actions) and abs(actions['time_seconds'][j] - start_time)< nr_seconds:
            ...
            if y['goal'][j] and y['team_id'][j] != possession_team: #other team scored a goal
                res.at[i] = True
                break
            elif y['owngoal'][j] and y['team_id'][j] == possession_team: #team conceded an owngoal
                res.at[i] = True
                break
            j += 1
    # print(pd.DataFrame(res, columns=['concedes']))
    return pd.DataFrame(res, columns=['concedes'])
