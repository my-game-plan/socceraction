...
from socceraction.hybrid_vaep import HybridVAEP
from socceraction.vaep import features as fs


@pytest.fixture(scope='session')
def vaep_model(sb_worldcup_data: pd.HDFStore) -> HybridVAEP:
    # Test the vAEP framework on the StatsBomb World Cup data
    model = HybridVAEP(nb_prev_actions=1)
    ...
    expected_features = set(fs.feature_column_names(model.xfns_standard, model.nb_prev_actions))
...
@pytest.mark.e2e
def test_predict(sb_worldcup_data: pd.HDFStore, vaep_model: HybridVAEP) -> None:
    games = sb_worldcup_data['games']
    game = games.iloc[-1]
...
@pytest.mark.e2e
def test_predict_with_missing_features(sb_worldcup_data: pd.HDFStore, vaep_model: HybridVAEP) -> None:
    games = sb_worldcup_data['games']
    game = games.iloc[-1]
    actions = sb_worldcup_data[f'actions/game_{game.game_id}']
